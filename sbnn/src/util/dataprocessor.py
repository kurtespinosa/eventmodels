...
def _filter_structures(instances, filter, GENERALISATION, INCLUDE_MENTION, PARTIAL_ARGUMENT_MATCHING):
    def _filter_out(trig_structures, filter, entities, triggers, generalisation, include_mention):
        ...
        def _transform_structure(structure, triggers, entities, generalisation,
                               include_mention, PARTIAL_ARGUMENT_MATCHING):
            new_inedges = []
            for i in structure:
                # rel = i
                # if len(rel) == 0:
                #     new_inedges.append((rel))
                # else:
                rel = i[0]
                id = i[1]
                type = ''
                ...
                new_inedges.append((rel, new_type))
            return new_inedges

        newtrigstructures = DefaultOrderedDict(list)

        for trig_id, structure in trig_structures.items():
            ...
            if structure == [()]:
                ...
                # Only one structure for SBNN so no loop
                # for structure in structures: #
                # inedges = []

                # remove the OUT edges, no IN_edges as well
                ...
                if len(structure) != 0:
                    # transform inedges
                    new_structure = _transform_structure(structure, triggers, entities, generalisation,
                                                 include_mention, PARTIAL_ARGUMENT_MATCHING)
                    ...
                    if trig_type in filter[generalisation]:
                        ...
                        list_inedges = _into_list(new_structure)

                        multiset_inedges = Counter(list_inedges)
    ...
    for i in range(len(instances)):
        ...
        if len(no_arg_triggers) > 0:
            empty_events = DefaultOrderedDict(list)
...
def create_vocabulary(instances, UNK_MIN_FREQUENCY, UNK_ASSIGNMENT_PROBABILITY):
    ...
    for instance in instances:
        ...
        for k, v in triggers.items():
            typ = v[0]
            _add_types_to_collection(arg_types, typ)
            word = v[3]
    ...
    word_type2id = OrderedDict()
    role_type2id = OrderedDict()
    # trigger_type2id = OrderedDict()
    # entity_type2id = OrderedDict()
    arg_type2id = OrderedDict()

    word_type2id[const.NONE_WORD_TYPE] = const.ID_NONE_WORD_TYPE
    for c in const.CONSTANT_WORDS:
        word_type2id[c] = len(word_type2id)

    word_count_list = words.most_common()
    for (word, count) in word_count_list:
        if count <= UNK_MIN_FREQUENCY: #TODO condition is >= or == or <? It should be <= because it is the min frequency where we use the UNK rep
    ...
    role_type2id[const.NONE_ROLE_TYPE] = const.ID_NONE_ROLE_TYPE
    role_type2id[const.UNK_TOKEN] = const.ID_UNK_ROLE_TYPE
    for (word, count) in word_count_list:
    ...
    # entity_type2id[const.NONE_ENTITY_TYPE] = const.ID_NONE_ENTITY_TYPE
    # for (word, count) in word_count_list:
    #     entity_type2id[word] = len(entity_type2id)
    # for trigger and entity types
    word_count_list = arg_types.most_common()
    arg_type2id[const.NONE_ARG_TYPE] = const.ID_NONE_ARG_TYPE
    for (word, count) in word_count_list:
        arg_type2id[word] = len(arg_type2id)


    # For generalisation levels
    level1_type2id = OrderedDict()
    level2_type2id = OrderedDict()
    level1_types, level2_types = util.get_unique_types_level()
    level1_type2id[const.NONE_LEVEL_1_TYPE] = const.ID_NONE_LEVEL_1_TYPE
    level2_type2id[const.NONE_LEVEL_2_TYPE] = const.ID_NONE_LEVEL_2_TYPE
    for typ in level1_types:
        level1_type2id[typ] = len(level1_type2id)
    for typ in level2_types:
        level2_type2id[typ] = len(level2_type2id)


    return word_type2id, role_type2id, arg_type2id, singleton, level1_type2id,level2_type2id

def instances_to_ids(instances, word_type2id, role_type2id, arg_type2id, USE_FILTER, data):
    def _instance2id(instance, word_type2id, role_type2id, arg_type2id):
        ...
        def arg2id(v, argtype2id, word2id):
            if v[0] not in argtype2id:
                ...
                val = argtype2id[v[0]]
            ids, count, total_cnt = _text2id(v[3].split(), word2id)
            return [val, v[1], v[2], ids, count, total_cnt]
        ...
        def _args2id(arguments, argtype2id, word2id):
            ...
            for k, v in arguments.items():
                ...
                _, _, _, _, count, total_cnt = arg2id(v, argtype2id, word2id)
                total_unk_cnt += count
                total_word_cnt += total_cnt
            return arg, total_unk_count, total_word_cnt

        def _relations_2id(relations, role_type2id):
            new_relations = OrderedDict()
            for id, triplet in relations.items():
                role = triplet[0]
                if role not in role_type2id:
                    role_id = const.ID_UNK_ROLE_TYPE
                    log.debug("Not supported: Missing types for Roles. %s", role)
                else:
        ...
        sentence, word_unk_count, total_word_cnt = _text2id(instance[const.PRED_SENTENCE_INFO_IDX][const.PRED_SENTENCE_IDX].split(), word_type2id)
        file_id = instance[const.PRED_SENTENCE_INFO_IDX][const.PRED_SENTENCE_FILE_ID_IDX]
        sen_id = instance[const.PRED_SENTENCE_INFO_IDX][const.PRED_SENTENCE_POS_IDX]
        arg_indices = instance[const.PRED_SENTENCE_INFO_IDX][const.IDS_SENTENCE_ARG_INDICES]
        sentence = [sentence, file_id, sen_id, arg_indices]

        triggers = instance[const.PRED_TRIGGERS_IDX]
        new_triggers, trig_unk_count, trig_word_count = _args2id(triggers, arg_type2id, word_type2id)

        entities = instance[const.PRED_ENTITIES_IDX]
        new_entities, ent_unk_count, ent_word_count = _args2id(entities, arg_type2id, word_type2id)
        ...
        unk_count = word_unk_count
        word_count = total_word_cnt
        ...
        return newinstance, unk_count, word_count

    new_instances = []
    total_unk_count = 0
    total_word_count = 0
    for i in range(len(instances)):
        instance_id, unk_count, word_count = _instance2id(instances[i], word_type2id, role_type2id, arg_type2id)
        total_unk_count += unk_count
        total_word_count += word_count
        new_instances.append(instance_id)
    log.info("%s: %s total words, %s (%.2f%%)  unk tokens", data, str(total_word_count), str(total_unk_count), total_unk_count/total_word_count*100)
    return new_instances
...
def generate_prediction_file_for_sbm(instances, predictions, filter, OUTPUT_DIR, all_file_ids, SPECIAL_ENTITIES, POST_FILTER):
    def _extract_event_structures_and_map_to_file_ids(predictions, instances):
        ...
        def _extract_event_trig_structures(predictions):
            ...
            def _extract_predictions(predictions):
                trig_structures = DefaultOrderedDict(list)
                ...
                for structure in structures:
                    ...
                    for rel in main_struct:
                        ...
                        try:
                            ...
                            log.error("Error: _extract_event_trig_structures", exc_info=True)
                        if edge[0] != const.ID_NONE_ROLE_TYPE:
        ...
        file_id_events_mapping = OrderedDict()
        ...
        for p in range(len(predictions)):
            ...
            cur_file_id_entities = OrderedDict()
            cur_file_id_triggers = OrderedDict()
            cur_trig_structures = DefaultOrderedDict(list)
    ...
    def _filter_predictions(file_id_events_mapping, filter, POST_FILTER):
                ...
                for rel in structure:
                    ...
                    if arg.startswith("E"):
                        try:
                            ...
                            log.error("Error converting to words", exc_info=True)
        ...
        new_file_id_events_mapping = OrderedDict()
        for quad in file_id_events_mapping:
            file_id = quad[0]
            entities = quad[1]
            triggers = quad[2]
            events = quad[3]
            new_events = DefaultOrderedDict(list)
            for event_id, structure in events.items():
                ...
                if POST_FILTER:
                    ...
                    # convert to multiset
                    structure_multiset = Counter(structure_in_words_before_numbering)
                    ...
                    if not found:
                        log.debug('post filter predictions not in the templates: %s:%s', structure, found)
                else:
                    # analysis
                    found = True
                ...
                if found:
                    # pick the structure which has an increasing numbering based on the id
    ...
    def filter_programmatically(inedges, trig_type):
        ...
        def is_valid(inedges, trigtype):
            ...
            if trig_type == 'Metastasis':
                ...
                binding1 = Counter(bindings[0])
                binding2 = Counter(bindings[1])
                event_set = Counter(event)
    ...
    def _convert_to_event_structures_and_assign_ids(file_id_events_mapping):
        ...
        def _create_event_ids(triggers, input_events):
            ...
            event_id = 1
            events = OrderedDict()
            trigger_event_mapping = DefaultOrderedDict(list)
            for trig, structures in input_events.items():
                for structure in structures:
                    if len(structure) == 0:
                        ...
                        # # # handle combination of structures: add a different event id
                        # new_events = _generate_numbered_roles(structure)
                        # for ev in new_events:
                        #     IN_structure = copy.deepcopy(ev)
                        #     IN_structure.insert(0, trig)
                        #     # IN_structure = [trig] + [ev]
                        #     new_event = "E" + str(event_id)
                        #     events[new_event] = IN_structure
                        #     trigger_event_mapping[trig].append(new_event)
                        #     event_id += 1
        ...
        def _replace_trigger_arg_with_event_ids(events_with_ids, trigger_event_mapping):
            def search_structure(same_structure, structure):
                ...
                for id, structures in same_structure.items():
                    ...
                    structure1_ = Counter(structure1)
                    structure_ = Counter(structure)
                    if structure1_ == structure_:
                        idx = id
                        break
                return idx
            ...
            same_trigger = DefaultOrderedDict(list)
            ...
            same_structure = DefaultOrderedDict(list)
            ctr = 0
            for event_id, structure in events_with_ids.items():
    ...
    def _adjust_ids(readable):
        ...
        file_id_trigger_ids = OrderedDict()
    ...
    def _fix_ne_id(readable, SPECIAL_ENTITIES):
        ...
        def _renumber_ne(ne, entities, max_trigger_id, SPECIAL_ENTITIES):
            ...
            new_ne_dict = OrderedDict()
            new_ne_id_mapping = OrderedDict()
            for id, defn in ne_dict.items():
                old_id = id
                if id in ne:
                    id = ne[id]
    ...
    log.debug("Generating prediction file from predictions: %s", len(predictions))
