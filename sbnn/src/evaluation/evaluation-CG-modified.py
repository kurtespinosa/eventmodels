...
def valid_argument_name(a):
    return re.match(r'^Cause|CSite|ToLoc|AtLoc|FromLoc|Site|(Theme|Participant|Instrument)\d*$', a) is not None
...
def valid_argument_type(arg, reftype):
    if arg == "Cause" or re.match(r'^Theme\d*$', arg):
        ...
        # TODO: narrow down, this is much too permissive in many cases
        return reftype in entity_types or reftype in event_types
    elif arg in ("ToLoc", "AtLoc", "FromLoc"):
        return reftype in given_types
    elif arg in ("CSite", ) or re.match(r'^Site\d*$', arg):
        return reftype in ("Protein_domain_or_region", "DNA_domain_or_region",
                           "Amino_acid")
    elif re.match(r'^Participant\d*$', arg):
        return reftype in entity_types
    elif re.match(r'^Instrument\d*$', arg):
        return reftype in entity_types
    else:
...
class Event:
    ...
    def resolve_idrefs(self, annotation_by_id):
        ...
        for arg, aid in self.args:
            ...
            m3 = re.match(r'^(Instrument)(\d*)$', arg)
            if m:
                ...
                self.instruments.append((argnum, ref))
            else:
                assert arg not in self.arg, "ERROR: event %s has multiple %s arguments" % (self.id, arg)
                self.arg[arg] = ref
                #EDITED
                # if arg in self.arg:
                #     print("ERROR: event %s has multiple %s arguments" % (self.id, arg))
                # else:
                #     self.arg[arg] = ref
        ...
        for argnum in theme_site_nums:
            ...
            if not options.spliteventeval and self.type != 'Mutation':
                ...
                #EDITED
                # if t_arg not in theme_site_dict:
                #     print("ERROR: event %s has Site%s without Theme%s" % (self.id, argnum, argnum))
            ...
            if s_arg in theme_site_dict:
                ...
                #EDITED
                # if len(theme_site_dict[s_arg]) != 1:
                #     print("ERROR: event %s has multiple Site%s arguments, Site-Theme pairing is ambiguous" % (self.id, argnum))
            ...
            if t_arg in theme_site_dict:
                ...
                if not options.spliteventeval or self.type != 'Mutation':
    ...
    def to_notbid_string(self):
        ...
        for num, ref in self.instruments:
            argstrs.append("%s:%s" % ("Instrument"+num, ref.to_notbid_string()))
        for a in self.arg:
    ...
    def matches(self, e, match_theme_only=False, match_partial=False):
        ...
        for sp in self.instruments:
            for op in e.instruments:
                ...
                if sp[1].matches(op[1], options.partialrecursive):
                    ...
                    break
        for sp in [p for p in self.instruments if p not in matched_self_i]:
            self_only_args.append('Instrument'+sp[0])
        for op in [p for p in e.instruments if p not in matched_other_i]:
            other_only_args.append('Instrument'+op[0])
...
def parse_event_line(l, fn):
    ...
    if type not in event_types:
        print("FORMAT ERROR: disallowed type for event annotation: line '%s' in %s" % (l, fn))
...
def parse_reference_files(PMID, t2_tasksuffix=""):
    ...
    # read text
    txt_fn = os.path.join(options.refdir, PMID+".split.txt")
...
op.add_option("-d","--pred_directory",action="store",dest="preddir",metavar="DIRECTORY",default=None,help="the directory where standoff files with evaluation results are placed.")
...
PRED_DIR = options.preddir
directory = os.listdir(PRED_DIR)
for f in directory:
    args.append(f)
...
for PMID in PMIDs:
    ...
    for e in events:
        if e.has_matched():
            ...
            #EDITED
            # if e.type not in ['DNA_domain_or_region', 'Protein_domain_or_region']:
            #     type_false_positive[e.type] += 1
    ...
    #EDITED:
    # if sum(type_false_positive.values()) != false_positive:
    #     print("INTERNAL ERROR")
...
if options.outputdir is not None:
    ...
    for PMID in PMIDs:
        ...
        outtxtfn = os.path.join(options.outputdir, PMID+".split.txt")
...
for t in output_event_type_order:
    if "TOTAL" in t:
        ...
        if not options.singlepenalty:
            print (" %s    %4d ( %4d)    %5d ( %4d)   %6.2f   %6.2f   %6.2f" % (t[:MAX_TYPE_PRINT_LEN].center(MAX_TYPE_PRINT_LEN), gold_event_count, total_type_match_gold[t], answer_event_count, total_type_match_answer[t], recall, precision, F))
        else:
            print (" %s    %4d ( %4d/%4d)    %5d ( %4d/%4d)   %6.2f   %6.2f   %6.2f" % (t[:MAX_TYPE_PRINT_LEN].center(MAX_TYPE_PRINT_LEN), gold_event_count, total_type_match_gold[t], total_type_over_match[t], answer_event_count, total_type_match_answer[t], total_type_partial_match[t], recall, precision, F))
