...
from config import entity_types, given_types, event_types, output_event_type_order, subtotal_event_set
...
def valid_argument_name(a):
    return re.match(r'^Cause|CSite|ToLoc|AtLoc|FromLoc|Sidechain|Contextgene|Participant|(Theme|Site)\d*$', a) is not None
...
def valid_argument_type(arg, reftype):
    ...
    elif arg in ("CSite", "ToLoc", "AtLoc", "FromLoc") or re.match(r'^Site\d*$', arg):
        if (arg in ("CSite",) or re.match(r'^Site\d*$', arg)) and reftype == "Part-site":
            return True
        return reftype in ("Entity", "Location")
    elif arg in ("Sidechain",):
        return reftype == "Entity"
    elif arg in ("Contextgene",):
        return reftype == "Protein"
    elif arg in ("Participant"):
...
class Equiv:
    ...
    def resolve_idrefs(self, annotation_by_id):
        ...
        for id in self.ids:
            ...
            # only given types can have Equivs
