...
class EventStructureClassifier(Loader):
    def __init__(self, n_word_types, n_trig_types, n_role_types, n_entity_types, trigger_type2id, entity_type2id,
        ...
        with self.init_scope():
            ...
            self.noevent = chainer.Parameter(I.Normal(1.0), (1, self.DIM_EVENT))
            # self.nomention = chainer.Parameter(I.Normal(1.0), (1, self.DIM_BILSTM * 2))
            # self.noargevent = chainer.Parameter(I.Normal(1.0), (1, self.DIM_EVENT))
    ...
    def _represent_type_and_argument(self, batch_i, type_index, bilstm_i, type_label, structures_above_threshold=None):
        ...
        if structures_above_threshold is not None:
            ...
            if type_index == const.IDS_TRIGGERS_IDX:
                if self.REPLACE_TYPE:
                    ...
                    new_arg = Util.extract_category(trig_word, self.GENERALISATION, const.TYPE_GENERALISATION, VERBOSE)
            ...
            mention_embedding = self._represent_mentions(mention_ids, bilstm_i)
    ...
    def _represent_mentions(self, mention_ids, bilstm_i):
        try:
            id = mention_ids[0]
            bi = bilstm_i[id]
        except:
    ...
    def _event_structure_network(self, trig_id, structure, bilstm_i, batch_i, structures_above_threshold=None):
        ...
        def _construct_relation_embedding(trig_embedding, relation, batch_i, bilstm_i, structures_above_threshold=None):
            ...
            if is_trigger:
                arg_embedding = self._represent_type_and_argument(batch_i, const.IDS_TRIGGERS_IDX, bilstm_i, arg, structures_above_threshold)
            else:
                arg_embedding = self._represent_type_and_argument(batch_i, const.IDS_ENTITIES_IDX, bilstm_i, arg)
        ...
        trig_embedding = self._represent_type_and_argument(batch_i, const.IDS_TRIGGERS_IDX, bilstm_i, trig_id)[0]

        tree_representation = []
        if structure == const.EMPTY_STRUCTURE or len(structure) == 0:
            #TODO: is this correct?
            treelstmrep = np.zeros((1, self.DIM_EVENT), dtype=np.float32)
            # treelstmrep = self.noargevent
    ...
    def _predict(self, instance, bilstm_i, target=None):
        def _add_to_predictions(trig, structure_id, structure_defn, pred, representation, predictions):
            ...
            if norm_pred.data > self.THRESHOLD:
            ...
            for relation in structure:
                ...
                if arg.startswith("TR"):
                    if arg in predictions:
                        ...
                        if io == const.IN_EDGE:  # IN relation
                            all_are = False
                            break
                        # else:
                        #     print("debug")
        ...
        def _extract_sub_event_predictions_combinations(structure, predictions):
            ...
            for relation in structure:
                ...
                io = relation[1]
                lst = []
                if arg.startswith("TR") and io == const.IN_EDGE and arg in predictions:
                    # if arg in predictions:
                    preds = predictions[arg]
        ...
        def represent_out_event(arg, batch_i, bilstm_i):
            # noevent = np.zeros((1, self.DIM_EVENT), dtype=np.float32)
            trig_embedding = self._represent_type_and_argument(batch_i, const.IDS_TRIGGERS_IDX, bilstm_i, arg)[0]
            event_representation = F.concat((self.noevent, trig_embedding))
            event_representation = self.reducedEvent(event_representation)
            return event_representation

        def _extract_representations(sub_event_prediction_combination, predictions, instance, bilstm_i):
            ...
            for i in sub_event_prediction_combination:
                ...
                if arg.startswith("TR"):
                    if arg in predictions:
                        ...
                        # print("handle OUT relation sub-events")
                        representation = represent_out_event(arg, instance, bilstm_i)
        ...
        for level in range(len(instance_events)):
            for trig, structures in instance_events[level].items():
                # if file_id == 'PMID-20124444' and trig == 'TR77':
                #     print("debug")
                    ...
                    if level == 0:
                        ...
                        if _all_sub_events_are_events(structure_id, predictions):
                            ...
                            for sub_event_prediction_combination in sub_event_predictions_combinations:
                                sub_event_representations = _extract_representations(sub_event_prediction_combination, predictions, instance, bilstm_i)
    ...
    def __call__(self, batch, target=None):
        ...
        return batch_predictions, batch_loss, batch_count
